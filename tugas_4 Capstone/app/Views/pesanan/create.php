...
$old = $old ?? [];
$errors = $errors ?? [];
$hargaPerHari = $harga_per_hari ?? (int) ($paket['harga_per_hari'] ?? 0);
?>
<main class="max-w-6xl mx-auto px-4 py-8">
    <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
        ...
        <div class="mb-6">
            ...
            <h2 class="text-xl font-semibold"><?= esc($paket['nama_paket']); ?></h2>
            <p class="text-slate-600"><?= esc($paket['deskripsi']); ?></p>
        </div>
        <?php else : ?>
        ...
        <form class="grid gap-4" method="POST" action="<?= route_url('pesanan/store'); ?>" id="formPemesanan">
            <input type="hidden" name="paket_id" value="<?= (int) ($paket['id'] ?? $paketId); ?>">
            ...
            <div>
                ...
                <input class="mt-2 w-full border border-slate-200 rounded-lg px-3 py-2 bg-slate-100" type="text"
                    value="<?= esc($paket['nama_paket'] ?? 'Belum dipilih'); ?>" readonly>
            </div>
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input class="mt-2 w-full border border-slate-200 rounded-lg px-3 py-2" type="text"
                        name="nama_pemesan" value="<?= esc(old_input($old, 'nama_pemesan')); ?>" required>
                </div>
                <div>
                    ...
                    <input class="mt-2 w-full border border-slate-200 rounded-lg px-3 py-2" type="text" name="no_hp"
                        value="<?= esc(old_input($old, 'no_hp')); ?>" required>
                </div>
            </div>
            ...
            <div class="grid gap-4 md:grid-cols-3">
                <div>
                    ...
                    <input class="mt-2 w-full border border-slate-200 rounded-lg px-3 py-2" type="date"
                        name="tanggal_pesan" value="<?= esc(old_input($old, 'tanggal_pesan')); ?>" required>
                </div>
                <div>
                    ...
                    <input class="mt-2 w-full border border-slate-200 rounded-lg px-3 py-2" type="number"
                        ...
                        value="<?= esc((string) old_input($old, 'lama_perjalanan', '1')); ?>" required>
                </div>
                <div>
                    <label class="text-sm font-semibold">Jumlah Peserta</label>
                    <input class="mt-2 w-full border border-slate-200 rounded-lg px-3 py-2" type="number"
                        name="jumlah_peserta" min="1"
                        value="<?= esc((string) old_input($old, 'jumlah_peserta', '1')); ?>" required>
                </div>
            </div>
