...
function old_edit_value($key, $default = "") {
    global $old, $pesanan;
    return isset($old[$key]) ? $old[$key] : $pesanan[$key] ?? $default;
}

$old_layanan = $old["layanan"] ?? [
    $pesanan["layanan_penginapan"] ? "penginapan" : null,
    $pesanan["layanan_transportasi"] ? "transportasi" : null,
    $pesanan["layanan_makan"] ? "makan" : null,
];
$old_layanan = array_filter($old_layanan);
?>
<!DOCTYPE html>
<html lang="id">
    ...
    <main class="max-w-6xl mx-auto px-4 py-8">
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
            <div class="mb-6">
                ...
                <h2 class="text-xl font-semibold"><?php echo htmlspecialchars($pesanan["nama_paket"]); ?></h2>
            </div>
            ...
            <form class="grid gap-4" method="POST" action="proses_update.php" id="formEdit">
                <input type="hidden" name="id" value="<?php echo (int) $pesanan["id"]; ?>">
                <input type="hidden" name="paket_id" value="<?php echo (int) $pesanan["paket_id"]; ?>">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <input class="mt-2 w-full border border-slate-200 rounded-lg px-3 py-2" type="text"
                            name="nama_pemesan" value="<?php echo htmlspecialchars(old_edit_value("nama_pemesan")); ?>"
                            required>
                    </div>
                    <div>
                        ...
                        <input class="mt-2 w-full border border-slate-200 rounded-lg px-3 py-2" type="text" name="no_hp"
                            value="<?php echo htmlspecialchars(old_edit_value("no_hp")); ?>" required>
                    </div>
                </div>
                ...
                <div class="grid gap-4 md:grid-cols-3">
                    <div>
                        ...
                        <input class="mt-2 w-full border border-slate-200 rounded-lg px-3 py-2" type="date"
                            name="tanggal_pesan"
                            value="<?php echo htmlspecialchars(old_edit_value("tanggal_pesan")); ?>" required>
                    </div>
                    <div>
                        ...
                        <input class="mt-2 w-full border border-slate-200 rounded-lg px-3 py-2" type="number"
                            ...
                            value="<?php echo htmlspecialchars(old_edit_value("lama_perjalanan", "1")); ?>" required>
                    </div>
                    <div>
                        <label class="text-sm font-semibold">Jumlah Peserta</label>
                        <input class="mt-2 w-full border border-slate-200 rounded-lg px-3 py-2" type="number"
                            name="jumlah_peserta" min="1"
                            value="<?php echo htmlspecialchars(old_edit_value("jumlah_peserta", "1")); ?>" required>
                    </div>
                </div>
                ...
                <div class="flex flex-wrap gap-3 pt-2">
                    ...
                    <a class="px-4 py-2 bg-slate-200 text-slate-700 rounded-lg hover:bg-slate-300"
                        href="modifikasi_pesanan.php">Batal</a>
                </div>
            </form>
        </div>
    </main>
